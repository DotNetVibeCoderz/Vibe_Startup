...
public class VehicleService
{
        ...
        int pageSize = 12)
    {
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            query = query.Where(v => v.Name.ToLower().Contains(s) || v.Brand.ToLower().Contains(s)
                || v.Model.ToLower().Contains(s) || v.PlateNumber.ToLower().Contains(s));
        }
    ...
    public async Task<Vehicle?> GetVehicleByIdAsync(int id)
        => await _db.Vehicles.Include(v => v.Photos).Include(v => v.Owner)
            .Include(v => v.Reviews).ThenInclude(r => r.User).FirstOrDefaultAsync(v => v.Id == id);

    public async Task<List<Vehicle>> GetPartnerVehiclesAsync(string ownerId)
        => await _db.Vehicles.Include(v => v.Photos).Include(v => v.Bookings)
            .Where(v => v.OwnerId == ownerId).OrderByDescending(v => v.CreatedAt).ToListAsync();
    ...
    public async Task<Vehicle?> UpdateVehicleAsync(Vehicle vehicle)
    {
        ...
        existing.UpdatedAt = DateTime.UtcNow; await _db.SaveChangesAsync();
        return existing;
    }
    ...
    public async Task<List<Vehicle>> GetPopularVehiclesAsync(int count = 6)
        => await _db.Vehicles.Include(v => v.Photos).Where(v => v.IsAvailable && v.IsVerified)
            .OrderByDescending(v => v.RentalCount).Take(count).ToListAsync();
    ...
    public async Task<List<Vehicle>> GetPendingVerificationAsync()
        => await _db.Vehicles.Include(v => v.Photos).Include(v => v.Owner)
            .Where(v => !v.IsVerified).OrderBy(v => v.CreatedAt).ToListAsync();
    ...
    public async Task<object> GetVehicleStatsAsync()
        => new
        {
            ...
            AvailableVehicles = await _db.Vehicles.CountAsync(v => v.IsAvailable),
            TotalCars = await _db.Vehicles.CountAsync(v => v.Type == VehicleType.Car),
            TotalMotorcycles = await _db.Vehicles.CountAsync(v => v.Type == VehicleType.Motorcycle),
            PendingVerification = await _db.Vehicles.CountAsync(v => !v.IsVerified),
        };
}
